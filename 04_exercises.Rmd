...
ggmap(twincities) + # creates the map "background"
  geom_point(data = Starbucks, 
             ...
             alpha = 1, 
             size = 2) +
  annotate(geom = "text", x = 44.9416, y = -93.16, label = "Macalester")  +
  labs(title = "Starbucks in the Twin Cities") +
  theme_map()
```
...
pal <- colorNumeric("magma",
                    domain = starbucks_with_2018_pop_est$starbucks_per_10000)

leaflet(starbucks_with_2018_pop_est) %>%
  addTiles() %>% 


(fillColor = ~pal(starbucks_per_10000), 
              stroke = FALSE, 
              label = ~paste(str_to_tile(state), 
                             ":", 
                             round(starbucks_per_10000,0), 
                             .sep = ""), 
